...
# ОТВЕТИТЬ: а разобрались, как и почему этот вариант работает?
...
# УДАЛИТЬ: эта переменная не используется — в её создании нет необходимости
index_list = []
# УДАЛИТЬ: эта переменная используется только единожды — в её создании нет необходимости
list_tuples = enumerate(list_num)

# ИСПОЛЬЗОВАТЬ: многострочную запись для более сложных генераторных выражений и распаковку в переменные цикла:
# index_list = [el[0] for el in list_tuples if el[1] in sublist_num]
index_list = [
    i
    for i, num in enumerate(list_num)
    if num in sublist_num
]
result_list_num = []
for i in index_list:
    n = 1
...
# УДАЛИТЬ: в списке result_list_num и в проверке ниже нет необходимости — достаточно удостовериться, что в списке индексов разность между каждой парой элементов составляет ровно единицу
if result_list_num == sublist_num:
    result = 'Да'
print(result)

# КОММЕНТАРИЙ: хороший, кстати, способ — поздравляю, не зря время потратили
