user_coord_1 = input('Ладья. Введите координату откуда ходим: ').lower()
user_coord_2 = input('Введите координату куда ходим: ').lower()

if user_coord_1[0] == user_coord_2[0] or user_coord_1[1] == user_coord_2[1]:
    print('Ход верный')
else: 
    print('Ход не верный')
# Ладья. Введите координату откуда ходим: a1
# Введите координату куда ходим: h1
# Ход верный

# Ладья. Введите координату откуда ходим: e3
# Введите координату куда ходим: f4
# Ход не верный
# --------------------------------------------------------------

# user_coord_1 = input('Ладья. Введите координату откуда ходим:').lower()
# user_coord_2 = input('Введите координату куда ходим:').lower()

# УДАЛИТЬ: эти переменные используются каждая только единожды — в их создании нет необходимости
# coord_y_1 = user_coord_1[1]
# coord_x_1 = user_coord_1[0]
# coord_y_2 = user_coord_2[1]
# coord_x_2 = user_coord_2[0]

# if coord_y_1 == coord_y_2 or coord_x_1 == coord_x_2:
    # print('Ход верный')
# else: 
    # print('Ход не верный')


# Ладья. Введите координату откуда ходим:B4
# Введите координату куда ходим:h4
# Ход верный

# Ладья. Введите координату откуда ходим:b4
# Введите координату куда ходим:c3
# Ход не верный


# ИТОГ: очень хорошо — 3/4
